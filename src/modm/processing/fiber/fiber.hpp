...
/**
 * Fiber is a cooperative threading model consisting of an execution context
 * (stack) and a function pointer representing the entry function.
 *
...
class Fiber {
 ...
 public:
  ...
  /**
   * Logs the fiber stack if logging is enabled, otherwise does nothing. Useful
   * for debugging.
   */
  void dumpStack();
 ...
 private:
  ...
  // Removes the current fiber from the scheduler and yields execution. This is
  // called on fiber return.
  static inline void done();

  modm_always_inline Fiber* next() { return next_; }
...
class Waitable {
 protected:
   ...
   * message passing (the reciever is guaranteed to consume the sent message
   * immediately).
   */
  void signal();
...
class Channel : Waitable {
 public:
   ...
   *
   * This method will be non-blocking if the channel is in the ready state,
   * otherwise it will yield execution.
   *
...
/**
 * A Semaphore is a lightweight synchronization primitive that can control
 * access to a shared resource. It allows up to MaxValue fibers to access the
...
class Scheduler {
 ...
 public:
  ...
  inline Fiber* currentFiber() { return current_fiber_; }

  inline Fiber* lastFiber() { return last_fiber_; }

  inline bool empty() { return last_fiber_ == nullptr; }
  ...
  inline void runNext(Fiber* fiber) {
    Fiber* current = currentFiber();
 ...
 protected:
  inline void registerFiber(Fiber*);
